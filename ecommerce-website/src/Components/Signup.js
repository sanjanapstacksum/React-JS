      ...
      createUserWithEmailAndPassword(auth, email, password)
        ...
        .catch((error) => {
          if (error.message == "Firebase: Error (auth/invalid-email).") {
            setErrorMsg("Please fill all the reqired fields");
          }
      ...
      <MDBRow style={{ display: "flex", justifyContent: "center" }}>
        ...
        <MDBCol col="4" md="4" style={{ alignItem: "center" }}>
          ...
          <MDBInput
            ...
            size="lg"
          />
          <span style={{ display: !name ? 'block' : 'none',color:"red"}}>{errorMsgDiv?.name}</span>
          <span style={{  display: !name ? 'none' : 'block',color:"red"}}>{regEx?.name}</span>
          ...
          <MDBInput
            ...
            size="lg"
          />
          <span style={{  display: !number ? 'block' : 'none',color: "red" }}>{errorMsgDiv?.number}</span>
          <span style={{ display: !number ? 'none' : 'block',color: "red" }}>{regEx?.number}</span>
          ...
          />
          <span style={{display: !email ? 'block' : 'none', color: "red" }}>{errorMsgDiv?.email}</span>
          <span style={{display: !email ? 'none' : 'block', color: "red" }}>{regEx?.email}</span>
          ...
          />
          <span style={{display: !password ? 'block' : 'none',color: "red" }}>{errorMsgDiv?.password}</span>
